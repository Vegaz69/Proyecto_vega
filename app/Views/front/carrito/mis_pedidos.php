        ...
        <?php foreach ($pedidos as $pedido): ?>
            <div class="card shadow-lg mb-4 border-0 rounded-3">
                <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center rounded-top-3">
                    <h5 class="mb-0 fw-bold">Pedido #<?= esc($pedido['id_venta']) ?></h5>
                    <span>Fecha: <?= date('d/m/Y H:i', strtotime(esc($pedido['fecha_venta']))) ?></span>
                </div>
                <div class="card-body">
                    <h6 class="card-subtitle mb-2 text-muted">Estado: <span class="badge bg-info"><?= esc(ucfirst($pedido['estado_venta'])) ?></span></h6>
                    <h6 class="card-subtitle mb-3 text-muted">Total: <span class="fw-bold text-success">$<?= number_format(esc($pedido['total_venta']), 2, ',', '.') ?></span></h6>
