...
<div class="container my-3">
    ...
    <?php if (session()->getFlashdata('success_cart')): ?>
        <div class="alert alert-success alert-dismissible fade show animate__animated animate__fadeInDown" role="alert">
            <i class="bi bi-check-circle-fill me-2"></i> <?= esc(session()->getFlashdata('success_cart')); ?>
    ...
    <?php if (session()->getFlashdata('error_cart')): ?>
        <div class="alert alert-danger alert-dismissible fade show animate__animated animate__fadeInDown" role="alert">
            <i class="bi bi-exclamation-triangle-fill me-2"></i> <?= esc(session()->getFlashdata('error_cart')); ?>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php endif; ?>
                            ...
                            <table class="table table-striped align-middle mb-0">
                                ...
                                <tbody>
                                    <?php foreach ($productos as $producto): ?>
                                        <tr class="text-center">
                                            <td class="fw-semibold text-dark"><?= esc($producto['nombre_producto']) ?></td>
                                            <td>
                                                <img src="<?= base_url('uploads/productos/' . esc($producto['imagen'] ?? 'placeholder.png')) ?>" alt="<?= esc($producto['nombre_producto']) ?>" class="img-thumbnail" style="width: 50px; height: 50px; object-fit: cover;">
                                            </td>
                                            <td class="fw-bold text-primary">$<?= number_format($producto['precio_unitario'], 2, ',', '.') ?></td>
                                            <td>
                                                <form action="<?= base_url('carrito/actualizar') ?>" method="post" class="d-flex align-items-center justify-content-center">
                                                    <?= csrf_field() ?>
                                                    <input type="hidden" name="id_carrito" value="<?= esc($producto['id_carrito']) ?>">
                                                    <input type="number" name="cantidad" value="<?= esc($producto['cantidad']) ?>" min="1" max="<?= esc($producto['stock_actual'] ?? 999) ?>" class="form-control form-control-sm me-2 text-center" style="width: 70px;">
                                                    <button type="submit" class="btn btn-sm btn-outline-primary" title="Actualizar"><i class="bi bi-arrow-clockwise"></i></button>
                                                </form>
                                            </td>
                                            <td class="fw-bold text-primary">$<?= number_format($producto['subtotal'], 2, ',', '.') ?></td>
                                            <td>
                                                <form action="<?= site_url('carrito/eliminar/'.$producto['id_carrito']) ?>" method="post" style="display:inline;">
                                                    <?= csrf_field() ?>
