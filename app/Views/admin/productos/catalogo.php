...
<div class="container">
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-xl-10"> 
                        ...
                        <div class="row g-2 align-items-center">
                            <div class="col-12 col-sm-6 col-md-5">
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" name="search" placeholder="Buscar producto por nombre o marca" value="<?= esc($search ?? '') ?>">
                                </div>
                            </div>
                            ...
                            <div class="col-6 col-md-3 d-flex gap-2">
                                ...
                                <?php if (!empty($search) || !empty($selected_categoria)) : ?>
                                    <a href="<?= base_url('admin/productos') ?>" class="btn btn-outline-secondary btn-sm w-100">Limpiar</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </form>
                                    ...
                                    <th>
                                        <a href="<?= base_url('admin/productos?orden_stock=' . ($orden_stock == 'asc' ? 'desc' : 'asc')) ?>" 
                                           class="text-white text-decoration-none fw-bold d-inline-flex align-items-center gap-2">
                                            Stock
                                            <span style="font-size: 0.8rem;"><?= ($orden_stock == 'asc') ? '↑' : '↓' ?></span>
                                        </a>
                                    </th>
                            ...
                            <tbody>
                                <?php foreach ($productos as $producto) : ?>
                                    <tr>
                                        ...
                                        <td class="d-none d-sm-table-cell"><?= esc($producto['marca']) ?></td>
                                        <td class="d-none d-sm-table-cell"><?= esc($producto['categoria_nombre']) ?></td>
                                        <td class="d-none d-sm-table-cell">$<?= number_format($producto['precio'], 2, ',', '.') ?></td>
                                        <td>
                                            <?php
                                                $stock = esc($producto['stock']);
                                                $claseStock = ($stock > 10) ? 'text-success' : (($stock > 3) ? 'text-warning' : 'text-danger');
                                            ?>
                                            <span class="<?= $claseStock ?> fw-bold"><?= $stock ?></span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-grid gap-1 d-md-flex">
                                                <a href="<?= base_url('admin/productos/editar/' . esc($producto['id_producto'])) ?>" class="btn btn-warning btn-sm px-2 py-1">
                                                    <i class="fas fa-edit d-sm-none"></i> <span class="d-none d-sm-inline">Editar</span>
                                                </a>
                                                <a href="<?= base_url('admin/productos/eliminar/' . esc($producto['id_producto'])) ?>" class="btn btn-danger btn-sm px-2 py-1"
                                                   onclick="return confirm('¿Seguro que quieres eliminar este producto?');">
                                                    <i class="fas fa-trash-alt d-sm-none"></i> <span class="d-none d-sm-inline">Eliminar</span>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <div class="col-12">
                                <div class="card border-0 shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <p class="fw-bold mb-1"><?= esc($producto['nombre']) ?></p>
                                            <p class="text-muted mb-1">Marca: <?= esc($producto['marca']) ?></p>
                                            <p class="text-muted mb-1">Categoría: <?= esc($producto['categoria_nombre']) ?></p>
                                            <p class="text-muted mb-1">Precio: $<?= number_format($producto['precio'], 2, ',', '.') ?></p>
                                            <p class="text-muted mb-1">Stock: <span class="<?= $claseStock ?> fw-bold"><?= esc($producto['stock']) ?></span></p>
                                        </div>
                                    </div>
                                    <div class="d-grid gap-1 d-md-flex">
                                        <a href="<?= base_url('admin/productos/editar/' . esc($producto['id_producto'])) ?>" class="btn btn-warning btn-sm px-2 py-1">
            ...
            <?php else : ?>
                <div class="table-responsive mt-5"> <table class="table table-striped table-hover align-middle">
                        <thead class="table-dark">
                            <tr>
                        ...
                        <tbody>
                            <?php foreach ($productosEliminados as $producto) : ?>
                                <tr>
                                    <td class="text-center"><?= esc($producto['nombre']) ?></td>                                    <td class="d-none d-sm-table-cell"><?= esc($producto['marca']) ?></td>
                                    <td class="d-none d-sm-table-cell"><?= esc($producto['categoria_nombre'] ?? 'Sin Categoría') ?></td>
                                    <td>
