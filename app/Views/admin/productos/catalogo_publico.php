...
<div class="container my-3">
            ...
            <form method="GET" action="<?= base_url('catalogo') ?>"
                ...
                <div class="me-2 mb-2 mb-md-0">
                    ...
                    <select name="categoria" id="categoria-select" class="form-select form-select-sm">
                        ...
                        <option value="<?= esc($categoria['id_categoria']) ?>" <?=(isset($selected_categoria) &&
                            $selected_categoria==$categoria['id_categoria']) ? 'selected' : '' ?>>
                ...
                <div class="me-2 mb-2 mb-md-0">
                    ...
                    <select name="orden" id="orden-select" class="form-select form-select-sm">
                        ...
                        <option value="nombre_asc" <?=(isset($orderBy) && $orderBy==='nombre_asc' ) ? 'selected' : '' ?>
                            >Nombre (A-Z)</option>
                        <option value="nombre_desc" <?=(isset($orderBy) && $orderBy==='nombre_desc' ) ? 'selected' : ''
                            ?>>Nombre (Z-A)</option>
                        <option value="precio_asc" <?=(isset($orderBy) && $orderBy==='precio_asc' ) ? 'selected' : '' ?>
                            >Precio (Menor a Mayor)</option>
                        <option value="precio_desc" <?=(isset($orderBy) && $orderBy==='precio_desc' ) ? 'selected' : ''
            ...
            <div class="card h-100 shadow-sm product-card">
                <a href="<?= base_url('productos/' . esc($producto['id_producto'])) ?>">
                    <img src="<?= base_url('uploads/productos/' . esc($producto['imagen'])) ?>"
                        class="card-img-top product-img" alt="<?= esc($producto['nombre']) ?>">
                </a>
                <div class="card-body d-flex flex-column p-2">
                    ...
                    <h6 class="text-primary product-price mb-2">$
                        <?= number_format(esc($producto['precio']), 2, ',', '.') ?>
                    </h6>
                    <div class="mt-auto d-flex justify-content-between align-items-center">
                        <?php if ($producto['stock'] > 0): ?>
                        <span class="badge bg-success">En Stock</span>
                        <a href="<?= base_url('productos/' . esc($producto['id_producto'])) ?>"
                            class="btn btn-sm btn-primary product-detail-btn">Ver Detalles</a>
                        <?php else: ?>
    ...
    <style>
        ...
        .product-title {
            ...
            /* Un poco más de peso */
            min-height: 2.7em;
            /* Altura mínima para 2 líneas de texto */
            line-height: 1.35;
        }
        ...
        .d-flex.justify-content-center.mt-4 nav ul {
            ...
            margin: 0;
            display: flex;
            /* Para que los elementos se muestren en línea */
            flex-wrap: wrap;
        ...
        .d-flex.justify-content-center.mt-4 nav ul li {
            margin: 0 8px;
            /* ¡Más espaciado horizontal para que se separen bien! */
        }
        ...
        .d-flex.justify-content->center.mt-4 nav ul li span {
            ...
            color: #007bff;
            /* Color azul de enlace por defecto */
            font-weight: normal;
            /* Asegura peso de fuente normal */
            padding: 4px 0;
            /* Un poco de padding para que sea clicable */
        }

        .d-flex.justify-content-center.mt-4 nav ul li.active a,
        .d-flex.justify-content-center.mt-4 nav ul li.current span {
            font-weight: bold;
            /* La página activa en negrita */
            color: #0d6efd;
            /* Color azul de Bootstrap para el activo */
            text-decoration: underline;
            /* Subraya solo el número activo como en tu imagen */
        }
    </style>
