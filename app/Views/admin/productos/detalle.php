...
<div class="container my-3">
    ...
    <?php if (!empty($producto)): ?>
        ...
        <div class="container my-3">
            ...
            <hr>
            <?php
            // Obtener la descripción del producto del arreglo $producto
            $rawDescription = $producto['descripcion'] ?? '';

            // Normalizar saltos de línea: reemplazar \r\n y \r por \n
            $rawDescription = str_replace(["\r\n", "\r"], "\n", $rawDescription);

            // Verificar si la descripción no está vacía antes de intentar procesarla
            if (!empty($rawDescription)) {
                // Dividir el texto por secciones (doble salto de línea)
                $sections = array_filter(array_map('trim', explode("\n\n", $rawDescription)));
                $section_count = count($sections);
                $col_counter = 0; // Para rastrear si estamos en la primera o segunda columna de un par
            ?>
                <?php foreach ($sections as $sectionContent): ?>
                    ...
                    // Abrir un nuevo row cada 2 columnas
                    if ($col_counter % 2 === 0):
                    ?>
                        <div class="row">
                    <?php endif; ?>
                    ...
                    <?php
                    $col_counter++;
                    // Cerrar el row cada 2 columnas o si es la última columna
            ...
            addToCartForm.addEventListener('submit', function(event) {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        if (data.new_stock !== undefined && stockBadge) {
                            stockBadge.innerText = `${data.new_stock} en stock`;
                            stockBadge.classList.remove('bg-success', 'bg-danger');
                            if (data.new_stock === 0) {
                                stockBadge.classList.add('bg-danger');
                                stockBadge.innerText = 'Sin stock';
                                addToCartButton.disabled = true;
                                addToCartButton.innerHTML = '<i class="fas fa-box-open me-2"></i>Fuera de Stock';
                                if (cantidadInput) cantidadInput.max = 0;
                                if (lowStockMessageContainer) lowStockMessageContainer.remove();
                            } else {
                                stockBadge.classList.add('bg-success');
                                if (cantidadInput) cantidadInput.max = data.new_stock;
                                const currentLowStockMessage = document.getElementById('low-stock-message');
                                if (data.new_stock <= 5 && !currentLowStockMessage) {
                                    stockBadge.insertAdjacentHTML('afterend', '<span id="low-stock-message" class="ms-2 text-warning fw-bold"><i class="fas fa-exclamation-triangle"></i> ¡Pocas unidades!</span>');
                                } else if (data.new_stock > 5 && currentLowStockMessage) {
                                    currentLowStockMessage.remove();
                                }
                            }
                        }
                    } else {
