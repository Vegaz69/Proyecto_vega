...
<h2 class="mb-4 text-center fw-bold"><i class="fas fa-user-edit me-2"></i> Editar Usuario:
    <?= esc($usuario['nombre']) ?>
</h2>
        ...
        <form action="<?= base_url('admin/usuarios/guardar') ?>" method="post">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre"
                            value="<?= old('nombre', $usuario['nombre']) ?>" required <?=isset($disableFieldsForAdmin)
                            && $disableFieldsForAdmin ? 'disabled' : '' ?>>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="apellido" name="apellido"
                            ...
                            <?=isset($disableFieldsForAdmin) && $disableFieldsForAdmin ? 'disabled' : '' ?>>
            ...
            <div class="mb-3">
                ...
                <input type="email" class="form-control" id="email" name="email"
                    value="<?= old('email', $usuario['email']) ?>" required <?=isset($disableFieldsForAdmin) &&
                    $disableFieldsForAdmin ? 'disabled' : '' ?>>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="rol" name="rol" required <?=isset($disableFieldsForAdmin) &&
                    $disableFieldsForAdmin ? 'disabled' : '' ?>>
                    <?php
                    if (isset($roles_posibles) && is_array($roles_posibles)) :
                    ?>
                    <?php foreach ($roles_posibles as $rol_cadena) : ?>
                    <option value="<?= esc($rol_cadena) ?>" <?=(old('rol', $usuario['rol'])==$rol_cadena) ? 'selected'
                        : '' ?>>
                        <?php
                                    if ($rol_cadena === 'admin') {
                                        echo 'Administrador';
            ...
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="activo" name="activo" value="1" <?=(old('activo',
            ...
            <div class="d-flex justify-content-end gap-2">
                <button type="submit" class="btn btn-primary" <?=isset($disableFieldsForAdmin) && $disableFieldsForAdmin
                    ? 'disabled' : '' ?>>
