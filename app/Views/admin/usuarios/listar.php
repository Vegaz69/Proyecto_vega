...
<div class="row mb-4 justify-content-center gap-2">
    ...
    <div class="col-auto">
        <form method="get" class="d-flex flex-nowrap" role="search"> <input type="text" name="busqueda" value="<?= esc($busqueda ?? '') ?>" class="form-control form-control-sm me-1" placeholder="Buscar..." style="min-width: 150px;"> <button class="btn btn-outline-dark btn-sm" type="submit">
                <i class="fas fa-search"></i>
            </button>
            <?php if (!empty($busqueda)) : ?>
                <a href="<?= base_url('admin/usuarios') ?>" class="btn btn-outline-secondary btn-sm ms-1 text-nowrap"> <i class="fas fa-times"></i> Limpiar Filtro
                </a>
            <?php endif; ?>
        </form>
    </div>
...
<?php endif; ?>
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-xl-10"> 
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                <?php foreach ($usuarios as $usuario) : ?>
                                    <tr>
                                        <td class="d-none d-sm-table-cell text-center"><?= esc($usuario['nombre'] ?? '') ?></td>
                                        <td class="d-none d-sm-table-cell text-center"><?= esc($usuario['apellido'] ?? '') ?></td>
                                        <td class="text-center"><?= esc($usuario['email'] ?? '') ?></td>
                                        <td class="text-center">
                                            <?= ($usuario['rol'] ?? '') === 'admin' ? 'Administrador' : (($usuario['rol'] ?? '') === 'cliente' ? 'Cliente' : 'Desconocido') ?>
                                        </td>
                                        <td class="text-center">
                                            <div class="d-grid gap-1 d-md-flex justify-content-center"> <a href="<?= base_url('admin/usuarios/editar/' . esc($usuario['id_usuario'] ?? '')) ?>" 
                                                ...
                                                <a href="<?= base_url('admin/usuarios/desactivar/' . esc($usuario['id_usuario'] ?? '')) ?>" 
                                                   ...
                                                   onclick="return confirm('¿Estás seguro de que quieres desactivar a este usuario?');">
                                                    <i class="fas fa-user-slash d-sm-none"></i> <span class="d-none d-sm-inline">Desactivar</span>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <?php foreach ($usuarios as $usuario) : ?>
                        <div class="col-12">
                            <div class="card border-0 shadow-sm p-3">
                                <p class="fw-bold mb-1"><?= esc($usuario['email'] ?? '') ?></p>
                                <p class="text-muted mb-2"><?= ($usuario['rol'] ?? '') === 'admin' ? 'Administrador' : (($usuario['rol'] ?? '') === 'cliente' ? 'Cliente' : 'Desconocido') ?></p>
            ...
            <div class="d-none d-md-block mt-5"> <?php if (empty($usuariosInactivos)) : ?>
                                            ...
                                            <div class="d-grid gap-1 d-md-flex justify-content-center"> <a href="<?= base_url('admin/usuarios/editar/' . esc($usuario['id_usuario'] ?? '')) ?>" 
                                                ...
                                                <a href="<?= base_url('admin/usuarios/activar/' . esc($usuario['id_usuario'] ?? '')) ?>" 
                                                   ...
                                                   onclick="return confirm('¿Estás seguro de que quieres activar a este usuario?');">
                                                    <i class="fas fa-user-check d-sm-none"></i> <span class="d-none d-sm-inline">Activar</span>
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
            ...
            <div class="d-md-none mt-5"> <?php if (empty($usuariosInactivos)) : ?>
                    <div class="alert alert-info">No hay usuarios inactivos.</div>
                <?php else : ?>
                    <div class="row gy-3">
                        <?php foreach ($usuariosInactivos as $usuario) : ?>
