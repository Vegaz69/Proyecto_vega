...
<div class="container-fluid py-4">
    ...
    <div id="dynamicAlertContainer" class="position-fixed top-0 start-50 translate-middle-x p-3" style="z-index: 1050;">
    </div>
    ...
    <div class="d-flex flex-wrap justify-content-start align-items-end mb-4 p-3 bg-light rounded shadow-sm">
        <form id="filterForm" action="<?= base_url('admin/mensajes') ?>" method="get"
            ...
            <div class="me-3 mb-2 mb-md-0">
                <select class="form-select form-select-sm" id="tipo_mensaje" name="tipo_mensaje">
                    ...
                    <option value="cliente" <?=(isset($filtro_tipo) && $filtro_tipo=='cliente' ) ? 'selected' : '' ?>
                        >Cliente</option>
                    <option value="contacto" <?=(isset($filtro_tipo) && $filtro_tipo=='contacto' ) ? 'selected' : '' ?>
                        >Contacto</option>
                </select>
            </div>
            ...
            <div class="me-3 mb-2 mb-md-0">
                <select class="form-select form-select-sm" id="estado_mensaje" name="estado_mensaje">
                    ...
                    <option value="no_leido" <?=(isset($filtro_estado) && $filtro_estado=='no_leido' ) ? 'selected' : ''
                        ?>>Nuevos</option>
                    <option value="leido" <?=(isset($filtro_estado) && $filtro_estado=='leido' ) ? 'selected' : '' ?>
                        >Leídos</option>
                </select>
            </div>

            <div class="me-3 mb-2 mb-md-0">
                <input type="date" class="form-control form-control-sm" id="fecha_mensaje" name="fecha_mensaje"
                    value="<?= esc($filtro_fecha ?? '') ?>" title="Filtrar por Fecha">
            </div>
            ...
            <div class="mb-2 mb-md-0">
                <a href="<?= base_url('admin/mensajes') ?>" class="btn btn-secondary btn-sm"><i
                        class="fas fa-redo me-1"></i> Limpiar</a>
            </div>
        </form>
    </div>
    ...
    <div class="card shadow-lg mb-4">
        ...
        <div class="card-body" id="messagesTableContainer">
            ...
            <div class="messages-list-mobile-view">
                <div class="table-responsive">
                    <table class="table table-hover table-striped">
                        ...
                        <tbody>
                            ...
                            <tr id="message-row-<?= esc($mensaje['id_mensaje']) ?>"
                                class="<?= $mensaje['leido'] ? 'text-muted' : 'fw-bold'; ?>">
                                <td>
                                    <?php if (($mensaje['tipo_mensaje'] ?? null) == 'cliente'): ?>
                                    <span class="badge bg-primary"><i class="fas fa-user-circle me-1"></i>
                                        Cliente</span>
                                    <?php else: ?>
                                    <span class="badge bg-info text-dark"><i class="fas fa-paper-plane me-1"></i>
                                        Contacto</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    <?= esc($mensaje['asunto'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= esc($mensaje['nombre_remitente'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= esc($mensaje['email_remitente'] ?? 'N/A') ?>
                                </td>
                                <td>
                                    <?= esc($mensaje['fecha_envio_formato'] ?? 'N/A') ?>
                                </td>
                ...
                <div class="messages-list-mobile-triggers">
                    ...
                    <div class="message-item-trigger view-message-btn <?= $mensaje['leido'] ? 'text-muted' : 'fw-bold'; ?>"
                        id="message-trigger-<?= esc($mensaje['id_mensaje']) ?>" data-bs-toggle="modal"
                        data-bs-target="#messageModal" data-asunto="<?= esc($mensaje['asunto'] ?? 'Sin Asunto') ?>"
                        data-remitente="<?= esc($mensaje['nombre_remitente'] ?? 'Anónimo') ?>"
                        data-email="<?= esc($mensaje['email_remitente'] ?? 'N/A') ?>"
                        data-tipo="<?= esc($mensaje['tipo_mensaje'] ?? 'desconocido') ?>"
                        data-fecha="<?= esc($mensaje['fecha_envio_formato'] ?? 'N/A') ?>"
                        ...
                        <div class="message-content-wrapper">
                            <div>
                                <?php if (($mensaje['tipo_mensaje'] ?? null) == 'cliente'): ?>
                                <span class="badge bg-primary message-type-badge"><i
                                        class="fas fa-user-circle me-1"></i> Cliente</span>
                                <?php else: ?>
                                <span class="badge bg-info text-dark message-type-badge"><i
                                        class="fas fa-paper-plane me-1"></i> Contacto</span>
                                <?php endif; ?>
                            </div>
                            <strong class="d-block mt-1">
                                <?= esc($mensaje['asunto'] ?? 'Sin Asunto') ?>
                            </strong>
                        ...
                        <div class="message-status-text">
                            <?php if ($mensaje['leido']): ?>
                            <i class="fas fa-check-circle text-success message-status-icon"></i> <span
                                class="d-none d-sm-inline">Leído</span>
                            <?php else: ?>
                            <i class="fas fa-exclamation-circle text-warning message-status-icon"></i> <span
                                class="d-none d-sm-inline">Nuevo</span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
