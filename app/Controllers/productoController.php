...
public function buscar()
    {
        ...
        $query = $request->getGet('q'); // Obtenemos el término de búsqueda

        $data = [
            'searchTerm' => $query,
            'productos' => [], // Inicializamos la variable productos
            'pager' => null, // Inicializamos el paginador
        ];

        if ($query) {
            // Llamamos al método de búsqueda del modelo y obtenemos los productos paginados
            $productosEncontrados = $this->productoModel->buscarProductos($query, 8); // 8 productos por página, puedes ajustar

            $data['productos'] = $productosEncontrados;
            $data['pager'] = $this->productoModel->pager; // Obtenemos el objeto paginador

            if (empty($productosEncontrados)) {
                $data['mensaje'] = "No se encontraron resultados para '{$query}'.";
            } else {
                $data['mensaje'] = "Resultados de búsqueda para '{$query}'.";
            }
        } else {
            $data['mensaje'] = "Por favor, ingresa un término para buscar productos, marcas o categorías.";
        }

        // Carga las vistas para mostrar los resultados
        echo view('front/head_view', $data); // <--- CAMBIO AQUÍ
        echo view('front/nav_view');        // <--- CAMBIO AQUÍ también a nav_view por consistencia
        echo view('buscar_resultados', $data); // Aquí se muestran los productos
        echo view('front/footer_view');     // <--- CAMBIO AQUÍ también a footer_view por consistencia
    }
