...
class CarritoController extends Controller {
    ...
    if ($isAjax) {
        ...
        $this->productoModel->update($id_producto, ['stock' => $new_stock]);
        ...
        if ($isAjax) {
            ...
            return redirect()->to(base_url('carrito/ver')); // CORREGIDO AQUÍ
        }
    }
    ...
    public function ver() {
        ...
        return view('front/carrito/ver', $data); // Asegúrate de que esta sea la ruta correcta a tu vista
    }
    ...
    public function actualizar() {
        ...
        if ($cantidad < 1) {
            return redirect()->to(base_url('carrito/ver'))->with('error_cart', 'La cantidad debe ser al menos 1.'); // CORREGIDO AQUÍ
        }

        $producto_en_carrito = $this->carritoModel->find($id_carrito);

        if (!$producto_en_carrito || $producto_en_carrito['id_usuario'] != $id_usuario) {
            return redirect()->to(base_url('carrito/ver'))->with('error_cart', 'Producto no encontrado en el carrito o no tienes permiso.'); // CORREGIDO AQUÍ
        }

        $producto_real = $this->productoModel->find($producto_en_carrito['id_producto']);
        if (!$producto_real || $producto_real['stock'] < $cantidad) {
            return redirect()->to(base_url('carrito/ver'))->with('error_cart', 'No hay suficiente stock disponible para la cantidad solicitada. Stock actual: ' . ($producto_real['stock'] ?? 0)); // CORREGIDO AQUÍ
        }
        ...
        return redirect()->to(base_url('carrito/ver'))->with('success_cart', 'Cantidad actualizada.'); // CORREGIDO AQUÍ
    }
    ...
    public function confirmar_compra()
    {
        ...
        try {
            ...
            foreach ($items_carrito as $item) {
                $producto_stock = $this->productoModel->find($item['id_producto']);
                if (!$producto_stock || $producto_stock['stock'] < $item['cantidad']) {
                    session()->setFlashdata('error_cart', 'No hay suficiente stock de "' . esc($item['nombre_producto']) . '". Stock actual: ' . ($producto_stock['stock'] ?? 0) . '. Por favor, ajusta la cantidad en tu carrito.');
                    $db->transRollback();
                    return redirect()->to(base_url('carrito/ver'));
                }
            ...
            foreach ($items_carrito as $item) {
                ...
                // Obtener el stock actualizado (importante para la comprobación de desactivación)
                $producto_stock = $this->productoModel->find($item['id_producto']); // Recargar para asegurar el stock más reciente
                $nuevo_stock = $producto_stock['stock'] - $item['cantidad'];
                ...
                if ($nuevo_stock <= 0) {
                    $this->productoModel->update($item['id_producto'], [
                        'activo' => 0 // Asume que 'activo' es el campo para activar/desactivar
                    ]);
